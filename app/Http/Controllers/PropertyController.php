...
class PropertyController extends Controller
{
    ...
    /**
     * Admin: Edit Existing Property
     */
    public function adminUpdateProperty(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'phone_number' => ['sometimes', 'required', 'regex:/^(09\d{9}|\+639\d{9})$/'],
            'type' => 'sometimes|in:Owner,Agent,Broker',
            'property_name' => 'sometimes|string',
            'location' => 'sometimes|string',
            'unit_type' => 'sometimes',
            'unit_status' => 'sometimes',
            'price' => 'sometimes|numeric',
            'square_meter' => 'sometimes|numeric',
            'floor_number' => 'sometimes|integer',
            'parking' => 'sometimes',
            'property_status' => 'sometimes',
            'features_amenities' => 'nullable|json',
            'virtual_tour_link' => 'nullable|url',
            'lightbox2_media' => 'nullable|array',
'lightbox2_media.*' => 'nullable|mimes:jpeg,png,jpg,webp,mp4,mov,avi,mkv|max:512000',
        ...
        if (isset($data['features_amenities'])) {
            $data['features_amenities'] = json_encode(json_decode($data['features_amenities'], true));
        }
     ...
     */
    public function adminAddProperty(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'first_name' => 'required|string',
            'last_name' => 'required|string',
...
'lightbox2_media.*' => 'nullable|mimes:jpeg,png,jpg,webp,mp4,mov,avi,mkv|max:512000',
        ...
        $this->storeLightboxMedia($request, $property->id);
        return response()->json(['message' => 'Property added by admin.', 'property' => $property], 201);
    }
